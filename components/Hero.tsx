...
/**
 * Back to the original, uncropped illustration at every breakpoint — no
 * mobile-specific zoom/crop wrapper. A few rounds of trying to make the
 * hero "bigger" on phones (widening it beyond the viewport and clipping
 * the sides, then a static crop of the sides) all amounted to cutting
 * something out of the artwork, which isn't what was wanted: the full
 * illustration, uncropped, is the point. The section itself adds no
 * padding/margin/min-height around the image, so there's no extra white
 * space beyond what the image's own natural (short, wide) shape leaves —
 * that whitespace comes from the rest of the page layout, not from this
 * component forcing a size.
 */
